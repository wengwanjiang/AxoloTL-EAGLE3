...
from transformers import (
    ...
    # AutoEagle3DraftModel, AutoDraftModelConfig, ### eagle3 
...
from transformers.models.eagle3.modeling_eagle3 import Eagle3Model
...
class ModelLoader:
    ...
    def __init__(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        if cfg.eagle3 is not None:
            from transformers.models.eagle3.configuration_eagle3 import Eagle3Config
            self.auto_model_loader = Eagle3Model  # pylint: disable=invalid-name
        else:
    ...
    def _build_model(self) -> bool:
        ...
        if (
            ...
            elif self.cfg.eagle3 is not None:
                assert isinstance(self.cfg.eagle3.target_model_path, str), print('target model cant be init with random param, must be init with pretrained weights!!')
                from transformers.models.eagle3.configuration_eagle3 import Eagle3Config
                eagle3_config = Eagle3Config(draft_config=self.cfg.eagle3.draft_model_config,\
                                             target_config=self.cfg.eagle3.target_model_path,\
                                             length=self.cfg.eagle3.length)                
                self.model = self.auto_model_loader(eagle3_config)
            else:
